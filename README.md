...
**Phase 1-3: Understanding the document.** The pipeline first builds a mental model of the PDF. The *Skeleton* phase detects the table of contents and document structure. *TableScan* finds data tables (fee tables, ISIN listings). *ExternalRefScan* identifies references to documents that aren't the prospectus itself (KIIDs, annual reports) — this matters because the system needs to know when a value simply doesn't exist in this document.

**Phase 4-5: Figuring out what's in there.** The *Exploration* phase sends an agent to scan the document in parallel chunks (30 pages at a time by default), looking for fund mentions, page ranges, and relationships. *EntityResolution* then deduplicates — "JPMorgan Global Bond Fund" and "Global Bond Fund" are the same entity.

**Phase 6-7: Planning the work.** The *Logic* phase aggregates patterns without calling any LLM (pure Python logic — which funds share pages, which tables are "broadcast" tables that apply to all funds). The *Planning* phase then creates an extraction recipe for each fund: which pages to read, which fields to look for where.

**Phase 8-9: Doing the extraction.** The *Extraction* phase sends reader agents to pull actual values — ISINs, fees, constraints, objectives — with provenance. *Assembly* ties everything into the final hierarchical graph, runs gap-filling for umbrella-level fields, and computes quality metrics.

There's also an optional *FailureRecovery* phase that retries failed extractions with broader search strategies, and the *Critic* agent (enabled with `--critic`) that independently verifies extracted values and assigns confidence scores.
...
- **Critic** (`--critic`): Adds an independent verification step. Slower and more expensive, but catches hallucinations and assigns calibrated confidence scores. The critic now receives parsed tables as "ground truth" — if there's a discrepancy between LLM extraction and table parsing, the table wins.
